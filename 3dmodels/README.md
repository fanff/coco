...
| `leg_left.scad` | 1 | Left U-channel shin: hip horn at the top; foot shaft + 4 mm peg at the bottom |
...
| `ankle_preview.scad` | Left U-shin seated on the foot’s two anchors — **do not print** |
...
Each hip servo lives in a well on the base. The matching **leg** bolts onto the stock round horn. The shin is a **U-channel** that opens **outboard** (toward the side of the robot): two arms (fore and aft) and an inboard web. The arms sit on two shared **anchors** in the foot frame (`coco_foot_shaft_anchor` at the shaft / horn, `coco_foot_mirror_anchor` at the 4 mm front window). Orange and magenta pins in `foot_preview.scad` / `ankle_preview.scad` mark those points.
...
| `u_inner_d` | 28 mm | U-channel inside depth (Y, inboard) |
...
Tune `clearance` in `coco.scad` if a clone body is tight or sloppy. Tune `hip_span`, `leg_len`, `ankle_h`, `toe_reach`, `coco_toe_*`, `front_hole_d`, `hug_inset`, and `u_inner_d` there if you change stance, footprint, hole size, or shin section. The U’s fore–aft span follows `coco_foot_mirror_hole_x()`.
